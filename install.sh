...
if [ $os_check = 'Debian' ]; then
	if [ $check_major_release = 10 ]; then
		if [ `id -u` != 0 ] ; then
		...
		if [ -d /usr/src/vertcoin-p2pool-install.sh ]; then
			cd /usr/src/vertcoin-p2pool-install.sh
			git pull
		else
			git clone https://github.com/sjexoq/vertcoin-p2pool-install.sh			
		fi
		cd /usr/src/vertcoin-p2pool-install.sh/debian
		./install.sh $@
	else
		error "Although you are running $real_os we require version 10"
		os_unsupported
	fi
else
	os_unsupported
